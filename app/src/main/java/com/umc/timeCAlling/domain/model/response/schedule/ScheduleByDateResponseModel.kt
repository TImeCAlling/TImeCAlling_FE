...
data class ScheduleByDateResponseModel(
    ...
    val meetTime: MeetTimeResponseModel?,
    val name: String?,
    val repeatDays: List<String?>?,
    val scheduleId: Int?
) {
    fun toScheduleByDateResponseDto() = ScheduleByDateResponseDto(categories?.map{it?.toCategoriesResponseDto()}, checkListId, isWritten, isRepeat, meetTime?.toMeetTimeResponseDto(), name, repeatDays, scheduleId)
}
