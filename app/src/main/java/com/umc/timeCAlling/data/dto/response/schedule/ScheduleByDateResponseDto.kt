...
data class ScheduleByDateResponseDto (
    ...
    val meetTime: MeetTimeResponseDto?,
    val name: String?,
    val repeatDays: List<String?>?,
    val scheduleId: Int?
) {
    fun toScheduleByDateResponseModel() = ScheduleByDateResponseModel(categories?.map{it?.toCategoriesResponseModel()}, checkListId, isWritten, isRepeat, meetTime?.toMeetTimeResponseModel(), name, repeatDays, scheduleId)
}